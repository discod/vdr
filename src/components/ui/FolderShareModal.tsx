...
function ShareLinkTab({ folderId, dataRoomId, onGenerate, onRefetch }: { 
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      onRefetch();
      toast.success("Folder share link generated successfully");
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h4 className="text-md font-medium text-gray-900 mb-2">Generate Folder Share Link</h4>
        <p className="text-sm text-gray-600">
          Create a secure link to share this folder and all its contents. You can set expiration and access limits.
        </p>
      </div>
      ...
      {generatedLink && (
        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <h5 className="text-sm font-medium text-green-900 mb-2">Folder Share Link Generated</h5>
...
function InviteTab({ folderId, dataRoomId, onSend, onRefetch }: { 
  folderId: number | null;
  dataRoomId: number;
  onSend: any; 
  onRefetch: () => void;
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      onRefetch();
      reset();
      toast.success("Folder invitation sent successfully");
    } catch (error) {
      toast.error("Failed to send folder invitation");
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h4 className="text-md font-medium text-gray-900 mb-2">Send Folder Invitation</h4>
        <p className="text-sm text-gray-600">
          Send a direct invitation to someone's email address with access to this folder and its contents.
        </p>
      </div>
...
function ManageSharesTab({ shares, onRevoke, onRefetch }: { 
  ...
  const handleRevoke = async (shareId: number) => {
    if (confirm("Are you sure you want to revoke this folder share? The link will no longer work.")) {
      try {
        ...
        toast.error("Failed to revoke folder share");
      }
    }
  };
  ...
  if (shares.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <h4 className="text-lg font-medium text-gray-900 mb-2">No Active Folder Shares</h4>
        <p className="text-gray-600">
          You haven't created any shares for this folder yet.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        <h4 className="text-md font-medium text-gray-900 mb-2">Active Folder Shares</h4>
        <p className="text-sm text-gray-600">
          Manage existing folder shares and view access statistics.
        </p>
      </div>
                ...
                <div className="flex items-center space-x-2 mb-2">
                  {share.recipientEmail ? (
                    ...
                    <>
                      ...
                      <span className="text-sm font-medium text-gray-900">Folder Share Link</span>
                    </>
                  )}
...
interface FolderShareModalProps {
  folderId: number | null; // null for root folder
  folderName: string;
  dataRoomId: number;
  isOpen: boolean;
  onClose: () => void;
...
export function FolderShareModal({ folderId, folderName, dataRoomId, isOpen, onClose }: FolderShareModalProps) {
  const [activeTab, setActiveTab] = useState<"link" | "invite" | "manage">("link");
  const trpc = useTRPC();

  const { data: existingShares, refetch: refetchShares } = useQuery(
    trpc.getFolderShares.queryOptions({
      token: localStorage.getItem("auth-token") || "",
      folderId,
      dataRoomId,
    })
  ...
  const generateShareLink = useMutation(
    trpc.generateFolderShareLink.mutationOptions()
  );

  const sendFolderInvitation = useMutation(
    trpc.sendFolderInvitation.mutationOptions()
  );

  const revokeShareLink = useMutation(
    trpc.revokeFolderShareLink.mutationOptions()
  );
